...
最終更新: 2026-08-20 / 対象: `quick-calc.site/stars/`
...
| ブランチ | 中身 |
|---|---|
| `main` | `08080d4`（PR #17 まで。画面の修正3件を取り込み済み） |
| `gh-pages` | `main` と中身が同一（差分ゼロ・公開反映済み） |
...
### 済み: Supabase の差分SQL（2026-08-20 Hiroさんが実行）

`scripts/stars/generated/migrate-spot-columns.sql` は適用済み。
公開用の `stars_public_spots` が `city` / `caution` を返すことを本番で確認した（30件）。
申請フォームは座標なしで送れる状態になっている。

### 済み: 画面の修正3件（PR #17 でマージ・公開済み）

幅390pxでお知らせの閉じるボタンが押せない件（`z-index: 4`）、閉じたお知らせが
同じセッションで再び出る件（`stars/notice.js`）、地図ページの meta description に
「観測スポット」が無い件。本番の `stars.css` に `z-index: 4` が出ていることを確認した。
...
**このままでよい。** Hiroさんの方針は「公式での裏取りがベストだが必須ではない。
信頼できる複数の口コミで代替可能。ただしそれを明記する」（`scripts/stars/REVIEW-BRIEF-2.md`）。
差し替えを新しい作業として持ち出さないこと。

---

## 3. あとで対応すること

1. スポット名の canonicalName + aliases[] 対応（Hiroさんへ説明済み・採否は未定）
2. リポジトリの公開/非公開（下の「7」を参照）。**着手中**

### やらないこと（2026-08-20 Hiroさん指示・今後も提案しない）

- 保留7件・除外10件の再検討（47都道府県のうち17県が空欄のままでよい）
- 地名索引の補完（美ヶ原・洞爺湖・四国カルスト・野辺山）

掲載を増やす提案・地名を足す提案を、こちらから持ち出さないこと。
