...
**星見スポットサイト（`stars/`）だけを、新しいプライベートリポジトリへ分離する。**

Hiroさんの決定（2026-08-16）。目的は「**GitHub上でソースを読まれることをなくす**」こと。
**`calc-toolbox` 全体を非公開にするのではない。** 他の65個の計算ツールは公開のまま残す。
`calc-toolbox` は他のツールで使っているので、削除もしない。

Hiroさんは GitHub の有料会員。私有リポジトリからの Pages 配信ができる。

**この分離には、URLと履歴について先に決めることがある。「7. 分離の進め方」を必ず読むこと。**
**確認を取らずに着手しないこと。**
...
- **DNS は ConoHa**（`ns-a1〜a3.conoha.io`）。GitHub Pages が直接配信していて、前段にCDNは無い
- Hiroさんは **GitHub の有料会員**。私有リポジトリからの Pages 配信が可能
- GitHub Actions は**使っていない**（`.github/workflows` が無い）
- `service_role` キーはリポジトリに入っていない（`tools/poll/config.js` にあるのは注意書きのコメントだけ）
- **全198コミットのうち82コミットが `stars/` を触っている**
- `stars/` がリポジトリ内の外を参照しているのは5種類だけ

| 参照先 | 種類 |
|---|---|
| `../tools/poll/config.js` | **コードの依存**（Supabase接続情報）。新リポジトリへコピーが要る |
| `../contact.html?tool=stars` | ただのリンク。絶対URLにすればよい |
| `../`（計算ツールボックス） | 同上 |
| `../privacy.html` / `../disclaimer.html` | 同上 |
| `../shared/style.css` | `about.html` などが参照。stars 側は `stars.css` が主。要確認 |

### 先に決めること①: URL をどうするか

**Git にはフォルダ単位の公開設定が無い。** `stars/` だけを非公開にするには、
`stars/` を**別のリポジトリ**にするしかない。

そして **GitHub Pages は、1つのドメインを2つのリポジトリでパス分けできない。**
`quick-calc.site` は `calc-toolbox` が押さえているので、私有リポジトリ側は別のホスト名が要る。

| 案 | URL | 長所 | 短所 |
|---|---|---|---|
| **A（推奨）** | `stars.quick-calc.site` | GitHub だけで完結。追加の仕組み不要 | URLが変わる。`quick-calc.site/stars/` に転送を置く必要がある |
| **B** | `quick-calc.site/stars/` を維持 | **URLが変わらない**（SEO影響ゼロ） | **DNS を ConoHa から Cloudflare へ移す**必要がある。Cloudflare でパスごとに振り分ける。無料枠で可能だが仕組みが1つ増える |

**いまなら A の代償は小さい。** サイトは 2026-08-16 公開でまだ1日。
検索の評価がほとんど付いていないので、URLを移しても失うものが少ない。
**後になるほど A は高くつく。**

A を採る場合、`calc-toolbox` 側の `stars/` は削除し、代わりに転送用の小さな
`stars/index.html` を置く（`<meta http-equiv="refresh">` と `<link rel="canonical">`）。
GitHub Pages は本物の 301 を返せないので、転送はこの形になる。

### 先に決めること②: 過去の履歴をどうするか

**ここが一番の落とし穴。**

`stars/` を新しいリポジトリへ移し、`calc-toolbox` から消しても、
**`calc-toolbox` の履歴には残る**。82コミットぶんのソースが公開のまま読める。
つまり `score.js` は、分離した日の版がそのまま GitHub から取得できる。

| 案 | 内容 | 代償 |
|---|---|---|
| **X** | 履歴はそのまま。今後の変更だけが非公開になる | 分離時点の版は公開のまま読める。**目的は半分しか果たせない** |
| **Y** | `git filter-repo` で `calc-toolbox` の履歴から `stars/` を消し、force push | **全198コミットのハッシュが変わる**。他の作業ブランチ・手元のクローン・PRのリンクが壊れる。**他のツールで使っているリポジトリなので影響が大きい** |

**Hiroさんに X と Y のどちらかを確認してから着手すること。**
Y を採る場合は、実行前に `calc-toolbox` のバックアップ（`git clone --mirror`）を取る。

### 手順（Hiroさんの確認を取ってから実行すること）

1. 新しい**プライベート**リポジトリを作る（名前はHiroさんに決めてもらう）
2. `stars/` と `scripts/stars/` を、履歴つきで新リポジトリへ移す
   （`git filter-repo --path stars/ --path scripts/stars/` でこの2つだけを残したコピーを作る。
   **浅いコピーにしない。**「なぜそう作ったか」の記録が消える）
3. 依存している `tools/poll/config.js` を新リポジトリへコピーし、参照を直す
4. `../contact.html` などのリンクを絶対URL（`https://quick-calc.site/...`）に直す
5. 新リポジトリで Pages を有効化し、`CNAME` に `stars.quick-calc.site` を置く（案A の場合）
6. ConoHa の DNS に `stars` の CNAME レコードを足す
7. `calc-toolbox` 側の `stars/` を転送ページに差し替えて公開
8. **全テストを流す**（`verify.mjs` を幅3通り、`verify_live.mjs`）。
   新リポジトリでも Playwright が要る
9. `curl https://stars.quick-calc.site/` で実体を確認してから「移行済み」と報告
   → [[mistakes/claimed-deploy-without-live-check]]
10. 履歴の扱い（X か Y）を実施する

### 分離しても解決しないこと（説明済み・了解済み）

`score.js` はサイトからは隠せない。ブラウザで動く以上、開発者ツールから必ず読める。
非公開化で防げるのは「GitHub上でソースを読まれること」だけ。
Hiroさんはこれを承知のうえで、その効果を目的として分離を決めている。
