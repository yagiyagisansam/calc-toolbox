...
(function (global) {
  ...
  function search(query, limit) {
    ...
    /*
     * 集落・字の索引を混ぜるのは、主の索引が1件も返さなかったときだけ。
     *
     * 以前は「max に足りなければ足す」にしていたので、いったん読み込むと
     * 主の索引が1〜11件返す検索にも集落が混ざっていた。
     * 説明は「主の索引で0件のときだけ」なのに、実装がそうなっていなかった。
     * 読み込み済みかどうかで結果が変わる作りは、利用者にも読めない。
     */
    if (out.length === 0 && state.localReady) {
      out = searchLocal(q, max);
    }
    return out;
  }
